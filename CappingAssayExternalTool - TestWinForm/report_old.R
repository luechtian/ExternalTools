...
# Skyline results
skyline_report <- "CappingAssay_04_Report__Write_new_results_file_Capping_assay_summary.csv"
...
suppressWarnings(
  data <- read.csv(list.files(Sys.getenv("temp"), 
    ...
    rename(SampleName = SampleGroup,
           TotalAreaLight = light.TotalArea,
           TotalAreaHeavy = heavy.TotalArea) %>%
...
# Sample info 
if(length(list.files(pattern = "sample info.xlsx")) == 0){
  
  stop("Could not find 'sample info.xlsx'!")
...
if(any(str_detect(sample_results$CapType, "CC-AU|AU-Cap"))){
  
  sample_results_ccau <- sample_results %>% 
    filter(str_detect(CapType, "CC-AU|AU-Cap"), 
...
addWorksheet(wb, "raw data b-S-ARCA")
writeData(wb, "raw data b-S-ARCA", filter(data, Molecule == "b-S-ARCA"), 
          startCol = 1, startRow = 33, rowNames = FALSE, 
          headerStyle = rawdata_style, borders = "columns", withFilter = TRUE)

addWorksheet(wb, "raw data Dinucleotide")
writeData(wb, "raw data Dinucleotide", filter(data, Molecule == "Dinucleotide"), 
          startCol = 1, startRow = 33, rowNames = FALSE, 
          headerStyle = rawdata_style, borders = "columns", withFilter = TRUE)
...
addWorksheet(wb, "raw data ATP")
writeData(wb, "raw data ATP", filter(data, Molecule == "ATP pos"), 
          startCol = 1, startRow = 33, rowNames = FALSE, 
          headerStyle = rawdata_style, borders = "columns", withFilter = TRUE)

addWorksheet(wb, "raw data GTP")
...
fortification_table <- fortification %>% 
  select(Molecule, SampleName1, AccuracyFortification) %>% 
...
writeData(wb, "recovery", fortification_table)
...
getwd()
saveWorkbook(wb, "Capping-Assay_results_skyline.xlsx", overwrite = TRUE)
