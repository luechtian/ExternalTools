...
if(length(str_subset(list.files(".", pattern = ".xlsx", full.names = TRUE), "Capping-Assay_results")) > 1){
  
  error_msg <- simpleError("Too many files with same string 'Capping-Assay_results'")
  
  tryCatch(stop(error_msg), 
           finally = writeLines(
             paste0("Please specify Excel-results file to fill out: ",
                    "\n",
...
#   dplyr::distinct()
sample_information <- read.xlsx(textbox_path2, sep.names = " ") %>% 
  dplyr::distinct()
...
recovery_results <- data %>% 
  ...
  pivot_wider(names_from = Molecule,
              values_from = `MeanConcentration [µM]`) %>% 
  mutate(RNA = paste0(CapType, "; ", `AmountCap [mM]`, "mM", "; ", `RNALength [nt]`, " nt"),
         RNA = ifelse(str_detect(SampleName, "spiked"), "", RNA)) %>% 
  select(-`AmountCap [mM]`, -`CapType`, -`RNALength [nt]`, -`AmountRNA [ug]`) %>% 
...
saveWorkbook(wb, 
             if(length(str_subset(list.files(".", pattern = ".xlsx", full.names = TRUE), "Capping-Assay_results")) == 0){
               paste0(str_subset(list.files("..", pattern = ".xlsx", full.names = TRUE), "Capping-Assay_results"),
                      format(Sys.time(), "%Y-%m-%d_%H-%M-%S_"))
             } else {
               paste0(format(Sys.time(), "%Y-%m-%d_%H-%M-%S_"), 
                      str_subset(list.files(".", pattern = ".xlsx", full.names = F), "Capping-Assay_results"))
             }, 
             overwrite = TRUE)
